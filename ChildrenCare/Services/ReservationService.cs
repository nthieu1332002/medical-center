...
public class ReservationService : IReservationService
{
    ...
    public async Task<UpdateReservationContactDTO> GetReservationContactInfo(int reservationId)
    {
        ...
        return new UpdateReservationContactDTO()
        {
            ...
            PreservedDate = reservation?.PreservedDate?.ToString(GlobalVariables.DayFormat) ?? string.Empty,
            Hour = reservation?.PreservedDate?.Hour ?? 0,
            Minute = reservation?.PreservedDate?.Minute ?? 0,
    ...
    public async Task<CustomResponse> SubmitReservation(UpdateReservationContactDTO dto)
    {
        ...
        if (reservation.PreservedDate?.ToString(GlobalVariables.DayFormat) != dto.PreservedDate ||
            reservation.PreservedDate?.Hour != dto.Hour || reservation.PreservedDate?.Minute != dto.Minute)
        {
            try
            {
                var tempDate = DateTime.ParseExact(dto.PreservedDate + " " + dto.Hour + ":" + dto.Minute + ":00",
                    GlobalVariables.DateTimeFormat,
                    CultureInfo.InvariantCulture);
                reservation.PreservedDate = tempDate;
            }
            catch (Exception e)
            {
                return new CustomResponse(false, "Date Time Format is wrong");
            }
        }
